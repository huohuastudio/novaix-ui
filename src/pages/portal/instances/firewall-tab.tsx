...
export function FirewallTab({ instanceId, instanceBusy }: { instanceId: number; instanceBusy: boolean }) {
  ...
  return (
    <>
      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xs font-medium text-muted-foreground uppercase tracking-wider">防火墙规则</h2>
        ...
        {fw.rules.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-xs text-muted-foreground">暂无防火墙规则</p>
            <p className="text-xs text-muted-foreground/70 mt-1">添加规则来控制实例的网络访问</p>
          </div>
        ) : (
          <div className="rounded-2xl bg-background divide-y divide-border/50">
            {fw.rules.map((rule) => {
                ...
                <div key={rule.id} className="flex items-center justify-between px-5 py-3.5 gap-4">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="inline-flex items-center rounded-md px-2 py-0.5 text-2xs font-medium bg-muted">
                        {directionMap[rule.direction ?? ""] ?? rule.direction}
                      </span>
                      <span className={`inline-flex items-center rounded-md px-2 py-0.5 text-2xs font-medium ${action.className}`}>
                        {action.label}
                      </span>
                      {rule.protocol && (
                        <span className="text-xs text-muted-foreground font-mono uppercase">{rule.protocol}</span>
                      )}
                      ...
                      {rule.enabled === false && (
                        <span className="text-2xs text-muted-foreground/60">已禁用</span>
                      )}
                    </div>
                    {rule.description && (
