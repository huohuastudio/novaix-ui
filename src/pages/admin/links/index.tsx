...
import { LinkCreateSheet, LinkEditSheet } from "./link-form-sheet"

export default function Links() {
  const formatDate = useFormatDate()
  const [createOpen, setCreateOpen] = useState(false)
  const [editingItem, setEditingItem] = useState<LinkLinkItem | null>(null)
  ...
  const fetchData = useCallback(async ({ page, pageSize, sorting, filters }: FetchParams) => {
    ...
    const { data: res } = await getAdminLinks({
      query: {
        page,
        page_size: pageSize,
  ...
  const handleEdit = useCallback((item: LinkLinkItem) => {
    setEditingItem(item)
  }, [])

  const handleDelete = useCallback(async (item: LinkLinkItem) => {
    const ok = await confirm({
      title: "删除链接",
      description: `确定要删除链接「${item.name}」吗？此操作不可撤销。`,
      confirmText: "删除",
      destructive: true,
    })
    if (!ok) return
    await deleteAdminLinksById({ path: { id: item.id! } })
    table.refresh()
  }, [table, confirm])
  ...
  const columns: ColumnDef<LinkLinkItem>[] = useMemo(() => [
    {
      accessorKey: "id",
      header: "ID",
      enableSorting: true,
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        if (!logo) return "—"
        return <img src={logo} className="h-8 w-8 rounded object-cover" alt="" />
      },
    },
    {
