...
import { BannerCreateSheet, BannerEditSheet } from "./banner-form-sheet"

export default function Banners() {
  const formatDate = useFormatDate()
  const [createOpen, setCreateOpen] = useState(false)
  const [editingItem, setEditingItem] = useState<BannerBannerItem | null>(null)
  ...
  const fetchData = useCallback(async ({ page, pageSize, sorting, filters }: FetchParams) => {
    ...
    const { data: res } = await getAdminBanners({
      query: {
        page,
        page_size: pageSize,
  ...
  const handleEdit = useCallback((item: BannerBannerItem) => {
    setEditingItem(item)
  }, [])

  const handleDelete = useCallback(async (item: BannerBannerItem) => {
    const ok = await confirm({
      title: "删除轮播图",
      description: `确定要删除轮播图「${item.title}」吗？此操作不可撤销。`,
      confirmText: "删除",
      destructive: true,
    })
    if (!ok) return
    await deleteAdminBannersById({ path: { id: item.id! } })
    table.refresh()
  }, [table, confirm])
  ...
  const columns: ColumnDef<BannerBannerItem>[] = useMemo(() => [
    {
      accessorKey: "id",
      header: "ID",
      enableSorting: true,
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return image ? (
          <img src={image} className="h-10 w-16 rounded object-cover" alt={row.original.title ?? ""} />
        ) : "—"
      },
    },
    {
    ...
    },
    {
      ...
      header: "开始时间",
      enableSorting: false,
      cell: ({ row }) => row.original.start_at ? formatDate(row.original.start_at) : "—",
    },
    {
      accessorKey: "end_at",
      header: "结束时间",
      enableSorting: false,
      cell: ({ row }) => row.original.end_at ? formatDate(row.original.end_at) : "—",
    },
    {
  ...
  return (
    <div className="space-y-6">
      <DataTable
        ...
        emptyIcon={Image}
        emptyTitle="暂无轮播图"
        emptyDescription="添加轮播图以在首页展示"
        emptyAction={<Button variant="outline" onClick={() => setCreateOpen(true)}>新建轮播图</Button>}
        toolbar={
